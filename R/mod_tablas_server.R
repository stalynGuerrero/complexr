mod_tablas_server <- function(id, est_results, est_meta, dict) {
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$title <- shiny::renderText({
      i18n_t(dict(), "mod_tablas.title")
    })
    output$subtitle <- shiny::renderText({
      i18n_t(dict(), "mod_tablas.subtitle")
    })
    output$lbl_download_all <- shiny::renderText({
      i18n_t(dict(), "mod_tablas.download_all")
    })
    output$lbl_clear <- shiny::renderText({
      i18n_t(dict(), "mod_tablas.clear_all")
    })
    ...
    # ---- Capturar nuevo resultado de estimacion ----
      ...
      content = function(file) {
        ...
        switch(fmt,
          ...
          "txt"  = utils::write.table(
            combined, file, sep = "	", row.names = FALSE
          ),
    ...
    # ---- Registrar output DT + descarga por tabla (solo IDs nuevos) ----
    shiny::observe({
      items   <- tables_store$items
          ...
          output[[paste0("dl_", lid)]] <- shiny::downloadHandler(
            ...
            content = function(file) {
              ...
              switch(fmt,
                ...
                "txt"  = utils::write.table(
                  item$data, file, sep = "	", row.names = FALSE
                ),
    ...
    # ---- UI principal: pestanas dinamicas ----
    output$tabs_ui <- shiny::renderUI({
      ...
      # Etiquetas i18n de estimadores
      # (reutiliza claves del modulo de estimacion)
      ...
      tabs <- lapply(rev(names(items)), function(item_id) {
        ...
        var_part  <- if (nchar(meta$variable) > 0) {
          paste0(" \u00b7 ", meta$variable)
        } else {
          ""
        }
        ...
        shiny::tabPanel(
          ...
          # Selector de formato + boton de descarga individual
          shiny::div(
            style = paste0(
